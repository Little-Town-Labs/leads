...
import { createEmailSequence } from './actions';

type SequenceFormModalProps = {
  onClose: () => void;
};

export function SequenceFormModal({ onClose }: SequenceFormModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await createEmailSequence(formData);
    ...
    <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <h3 className="text-lg font-medium text-gray-900">Add Email Sequence</h3>
          <button
            onClick={onClose}
